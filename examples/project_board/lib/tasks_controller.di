class TasksController
  def self.form(request, context, id)
    ...
    task = if id == nil then Task.new({"project_id": "", "title": "", "done": 0}) else Task.find(db, id) end
    ...
    content = task_form_html(action, projects, task.project_id(), task.title(), task.done(), if id == nil then "Create task" else "Save task" end, context["csrf_token"])
    Div.html_response(200, layout_html(if id == nil then "New task" else "Edit task" end, content, context["current_user"], context["csrf_token"]))
  end
