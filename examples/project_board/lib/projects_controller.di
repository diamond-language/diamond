class ProjectsController
  ...
  def self.form(request, context, id)
    ...
    content = project_form_html(action, project.name(), project.description(), if id == nil then "Create project" else "Save project" end, context["csrf_token"])
    Div.html_response(200, layout_html(if id == nil then "New project" else "Edit project" end, content, context["current_user"], context["csrf_token"]))
  end
