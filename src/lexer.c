...
static DiamondTokenKind identifier_kind(const DiamondLexer *lexer) {
    ...
    if (text_equals(lexer, "protected", 9)) return DIAMOND_TOKEN_PROTECTED;
    ...
    if (text_equals(lexer, "delegate", 8)) return DIAMOND_TOKEN_DELEGATE;
...
DiamondToken diamond_lexer_next(DiamondLexer *lexer) {
    ...
    switch (character) {
        ...
        case '<':
            if (match(lexer, '=')) {
                if (match(lexer, '>')) return token(lexer, DIAMOND_TOKEN_SPACESHIP);
                return token(lexer, DIAMOND_TOKEN_LESS_EQUAL);
            }
