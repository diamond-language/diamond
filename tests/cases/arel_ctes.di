...
def run_tests()
  ...
  def test_cte_relation_builds_recursive_union_body()
    ...
    recursive_numbers = numbers.as("NUMBERS")
    anchor = Arel.from(seeds).project(seeds.column("value"))
    step = Arel.from(recursive_numbers).project(Arel.sql("value + 1"))
    body = numbers.recursive_body(anchor, step)
    sql, params = body.to_sql()
    Minitest.assert_equal("SELECT \"seeds\".\"value\" FROM \"seeds\" UNION ALL SELECT value + 1 FROM \"numbers\" AS \"NUMBERS\"", sql)
  end
