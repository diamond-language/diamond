...
def run_tests()
  ...
  def test_insert_select_accepts_ctes()
    ...
    sql, params = insert.to_sql()
    Minitest.assert_equal("WITH \"eligible\" AS (SELECT \"source_items\".\"name\" FROM \"source_items\" WHERE \"source_items\".\"qty\" > ?) INSERT INTO \"archived_items\" (\"name\") SELECT \"eligible\".\"name\" FROM \"eligible\"", sql)
    Minitest.assert_equal(1, params[0])
    insert.execute(db)
    Minitest.assert_equal("pens", db.query("SELECT name FROM archived_items")[0]["name"])
    db.close()
