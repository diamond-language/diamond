...
def run_tests()
  def test_core_expressions_have_deterministic_inspection()
    ...
    Minitest.assert_equal("DefaultValues", Arel.inspect(Arel::DefaultValues.new()))
    Minitest.assert_equal("ArelNode(unknown)", Arel.inspect(ThirdPartyArelNode.new()))
    roles = Arel.table("roles")
    join = Arel::Join.new(roles, people.column("role_id").eq(roles.column("id")), "LEFT OUTER")
    ...
    Minitest.assert_equal("Cte(active_people, ordinary, Query(from=people, projections=1, predicates=0, joins=0, ctes=0))", Arel.inspect(Arel::Cte.new("active_people", subquery)))
    Minitest.assert_equal("Cte(tree, recursive, Query(from=people, projections=1, predicates=0, joins=0, ctes=0))", Arel.inspect(Arel::Cte.new("tree", subquery, true)))
  ...
  def test_nodes_compare_structurally()
    ...
    Minitest.assert_equal(true, Arel.same?(Arel::DefaultValues.new(), Arel::DefaultValues.new()))
