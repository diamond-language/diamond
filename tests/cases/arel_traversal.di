...
def run_tests()
  ...
  def test_composition_children_can_be_replaced()
    ...
    join = Arel::Join.new(roles, people.column("role_id").eq(roles.column("id")), "INNER")
    ...
    changed_cte = Arel.with_children(Arel::Cte.new("ids", body), [
  ...
  def test_simplify_eliminates_double_negation_immutably()
    ...
    wrapped = Arel::Not.new(Arel::Not.new(predicate))
    ...
    nested = people.column("id").gt(0).and_also(Arel::Not.new(Arel::Not.new(predicate)))
    ...
    update = Arel.update(people).set({"active": true}).where(
      Arel::Not.new(Arel::Not.new(people.column("id").eq(4))))
    ...
    composed = Arel.simplify(Arel::Not.new(Arel::Not.new(policy_source)),
      [[policy_source, policy_replacement]])
    Minitest.assert_equal("Predicate(=, Attribute(people.verified), Bind(true))",
      Arel.inspect(composed))
  end
