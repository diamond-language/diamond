...
# Compares two Strings in constant time, for cases (Csrf below) that
# need it but aren't themselves an HMAC check the native HMAC.verify
# already covers. Built on HMAC.verify/HMAC.sha256 rather than a new VM
...
# constant_time_equal above, rejecting with 403 if it's missing or
# doesn't match. Checking a request header rather than parsing a form
# body is deliberate: this codebase has no built-in form-body parser to
# hook into (see packages/http's own README), and a header is also what
# an XHR/fetch-based client sends most naturally -- a traditional HTML
