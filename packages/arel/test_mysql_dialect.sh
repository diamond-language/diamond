...
# Opt-in conformance test for Arel::MySQLVisitor -- same reasoning as
# test_mariadb_dialect.sh: needs an already-running server Diamond can't
...
#
# Usage: bash test_mysql_dialect.sh
#   (or DIAMOND_BIN=../../build/diamond bash test_mysql_dialect.sh)

diamond="${DIAMOND_BIN:-diamond}"
cd "$(dirname "$0")"

if ! command -v podman >/dev/null 2>&1; then
    echo "podman not found -- skipping Arel::MySQLVisitor conformance test" >&2
    exit 0
fi

container_name="diamond-arel-mysql-dialect-test"
port=33064
...
podman run --rm -d --name "$container_name" \
    -e MYSQL_ROOT_PASSWORD=diamondtest \
    -e MYSQL_DATABASE=diamond_arel_test \
    -p "127.0.0.1:${port}:3306" \
    docker.io/library/mysql:8 >/dev/null

ready=0
for _ in $(seq 1 60); do
    if podman exec "$container_name" mysql -uroot -pdiamondtest -e "SELECT 1" >/dev/null 2>&1; then
        ready=1
        break
    fi
    sleep 1
...
# (see docs/io.md) -- same native class test_mariadb_dialect.sh's own
# fixture already uses, so the exported env var names are identical to
# that script's own.
