...
No threads, no OS-level scheduler, no changes to `packages/http`'s own
`http_parse_request`/`http_write_response` (used here completely
unmodified). Three pieces, all new:
...
- **One `Fiber` per connection**: `NonblockingConnection` wraps a
  ...
  `gremlin_serve`'s own loop is the only thing that ever calls `IO.poll`
  or `.resume` -- an ordinary single-threaded event loop, deciding which
  suspended connections have become ready again and waking exactly
  those.
