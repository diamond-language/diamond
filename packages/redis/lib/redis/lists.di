...
class RedisConnection
  def lpush(key, *values) -> Int = self.command("LPUSH", key, *values)
  def rpush(key, *values) -> Int = self.command("RPUSH", key, *values)
  def lpop(key) = self.command("LPOP", key)
  def rpop(key) = self.command("RPOP", key)
  def llen(key) -> Int = self.command("LLEN", key)
  ...
  def lrem(key, count, value) -> Int = self.command("LREM", key, count, value)
  ...
  def lrange(key, start = 0, stop = -1) -> Array = self.command("LRANGE", key, start, stop)
end
