...
- `textDocument/hover` (`lsp/hover.c`), `textDocument/definition`
  ...
  - All three also fall back to `lsp/receiver.c` for a method name
    ...
    used for type checks or opcode selection. `if`/`unless` joins preserve
    that fact only when both runtime paths carry the same fact (including when
    neither path reassigns the local); conflicting paths clear it rather than retaining
    the last branch compiled. Any other receiver this
