...
- Added typed fixed-arity instance-bound method wrappers, preserving declared
  parameters and returns through direct invocation and higher-order generic
  inference. Explicit generic bindings are substituted into instance and
  singleton method-reference wrapper contracts.
...
- Added same-outer-type parameterized unions such as
  `Array[Int] | Array[String]`, including inferred collection and control-flow
  joins, recursive runtime checks, and structural duplicate rejection.
...
- Added wall-clock and monotonic time APIs, calendar accessors, formatting,
  ...
  end-of-month clamping plus day/month boundary helpers that honor local DST.
